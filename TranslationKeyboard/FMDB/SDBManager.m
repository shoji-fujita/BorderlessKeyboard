...
#define kDefaultDBName @"words.db"
...
- (int) initializeDBWithName : (NSString *) name {
    ...
    if(!exist){
        NSString *resourcePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:name];
        exist = [fileManager copyItemAtPath:resourcePath toPath:_name error:&error];
    }
    if(!exist){
