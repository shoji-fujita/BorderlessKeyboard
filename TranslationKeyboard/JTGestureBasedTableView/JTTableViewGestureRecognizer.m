...
- (void)commitOrDiscardCell {
    ...
    if (cell.frame.size.height >= commitingCellHeight) {
        [self.delegate gestureRecognizer:self needsCommitRowAtIndexPath:self.addingIndexPath]; // セルを追加
    } else {
        [self.delegate gestureRecognizer:self needsDiscardRowAtIndexPath:self.addingIndexPath]; // セルを追加できず
    ...
    self.tableView.contentInset = UIEdgeInsetsMake(0, 0, 0, 0); // スクロール領域をコンテンツに合わせる
...
- (void)panGestureRecognizer:(UIPanGestureRecognizer *)recognizer {
    ...
    if ((recognizer.state == UIGestureRecognizerStateBegan
        ...
        && [recognizer numberOfTouches] > 0) { // TODO: ここのどこかにスライドストッパーを入れる
        ...
        //////////
        NSLog(@"%f, %f", location1.x, location1.y);
        ...
        CGPoint location2 = [recognizer locationOfTouch:0 inView:cell];
        NSLog(@"%f, %f", location2.x, location2.y);
        if (location2.y <0 || location2.y > 44) { // 今のセルから指が離れたら
            self.addingCellState = JTTableViewCellEditingStateMiddle;
        }
        ...
        if (fabsf(translation.x) >= commitEditingLength) {
            ...
            if (translation.x < 0) { // 追加 // 左スライドはキャンセル扱いにする
                [UIView beginAnimations:@"" context:nil];
        ...
        if (self.addingCellState == JTTableViewCellEditingStateMiddle) { // セル内から指が離れていたとき用
            [UIView beginAnimations:@"" context:nil];
            cell.contentView.frame = cell.contentView.bounds;
            [UIView commitAnimations];
        }
        ...
        cell.contentView.backgroundColor = RGB(222, 222, 222); // 追加
    }
}
